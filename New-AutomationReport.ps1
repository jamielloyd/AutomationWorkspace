...
.PARAMETER SendReportByEmail
    ...
    The default is to not send the report by email.

.EXAMPLE
    Create-AutomationReport `
        -NumberOfDaysForReport 1 `
        -OutputHTML $true `
        -SendReportByEmail $true

.NOTES
    AUTHOR: System Center Automation Team
    LASTEDIT: February 24, 2015
    v1: February 18, 2015
    v1.01: February 24, 2015 (support for new Azure cmdlets)
    v1.02: March 2, 2015 (support for change in new Azure cmdlets)
#>

workflow New-AutomationReport
{   
    param
    (
        ...
        [Parameter (Mandatory=$false)]
        [bool] $SendReportByEmail = $false,

        [Parameter (Mandatory=$false)]
        [string] $AutomationAccountName = 'default'
    ...
    if ($AutomationAccountName -eq 'default'){
        $AutomationAccountName = Get-AzureAutomationVariable -Name 'AutomationAccountName'
    }

    Write-Verbose "test"

    $AzureOrgIdCredential = Get-AutomationPSCredential -Name 'AzureOrgIdCredential'
    if ($SendReportByEmail) {$EmailCredential = Get-AutomationPSCredential -Name 'SmtpEmailCredential'}
    
    # Initialize dates (using UTC)
    ...
    {
        $EmailInfo = @{"ToAddress"="t-jafe@microsoft.com";`
                        "FromAddress"="jamie.feldman@live.ca";`
    ...
    $SubscriptionInfo = Get-AzureSubscription -Name $AzureSubscriptionName
    ...
    foreach ($Job in $Jobs)
    {
        if ($Job.Status -eq "Completed")
        {
            ...
            $errorstreams = Get-AzureAutomationJobOutput `
                            -Id $Job.Id `
                            -AutomationAccountName $AutomationAccountName `
                            -Stream Error
            if ($errorstreams) {Add-Member -InputObject $Job -NotePropertyName 'HasErrors' -NotePropertyValue $true}
        }
    }

    # Checkpoint (so jobs and runbooks do not get retrieved again if the runbook is suspended and resumed)
    # Initial value of the variable holdng the output is empty string
    Checkpoint-Workflow
    $Out = ""
...
"@
    ...
    function Report-ReportParameters
    {
        ...
        if ($OutputHTML)
        {
            ...
            "<tr><td><b>Subscription</b></td><td>" + $AzureSubscriptionInfo.SubscriptionName + "</td></tr>"
            "<tr><td><b>Automation Account</b></td><td>" + $AutomationAccountInfo.AutomationAccountName + "</td></tr>"
            "<tr><td><b>Account Location</b></td><td>" + $AutomationAccountInfo.Location + "</td></tr>"
            "</table>"
        }
        else
        {
    ...
    function Report-RunbooksWithJobs
    {
        ...
        foreach ($Job in $Jobs)
        {
            ...
            if (($Status -eq "Completed") -and ($Job.HasErrors))
            {
    ...
    function Report-ModifiedRunbooks
    {
        ...
        foreach ($Rb in $Rbs)
        {
            # Handle State and LastModifiedTime in new cmdlets vs old cmdlets (short-term fix until new cmdlets rolled out)
            if ($Rb.State)
            {
                # New cmdlets
                $RBState = $Rb.State
                # Convert to DateTime
                if ($Rb.LastModifiedTime -is [string])
                {
                    # Convert from String to DateTime
                    $LastModifiedTime = Get-Date $Rb.LastModifiedTime
                }
                else
                {
                    # Convert from DateTimeOffset to DateTime
                    $LastModifiedTime = $Rb.LastModifiedTime.DateTime
                }
            }
            else
            {
            ...
            # LastModifiedTime reflects when runbook was first created or when it was last published
            if (($LastModifiedTime -ge $DateTimeStart) -or ($RbState -eq "Edit") -or ($RbState -eq "New"))
            {
                ...
                {
                    $LastPublishedTime = $LastModifiedTime
                }
                else
                {
                    $LastPublishedTime = ""
                ...
                {
                    $OutTemp = "<tr>"
                    $OutTemp += "<td>" + $Rb.Name + "</td>"
                    $OutTemp += "<td>" + $RbState + "</td>"
                    $OutTemp += "<td>" + $Rb.Tags + "</td>"
                    $OutTemp += "<td>" + $LastPublishedTime + "</td>"
                    $OutTemp += "</tr>"
                }
                else
                {
                    $OutTemp = "`n`nName: " + $Rb.Name
                    $OutTemp += "`nState: " + $RbState
                    $OutTemp += "`nTags: " + $Rb.Tags
                    $OutTemp += "`nPublished (UTC): " + $LastPublishedTime
                }
                
                if ($RbState -eq "Published")
                {
                    $OutPublished += $OutTemp
                }
                elseif ($RbState -eq "New")
                {
                    $OutNew += $OutTemp
                }
                else
